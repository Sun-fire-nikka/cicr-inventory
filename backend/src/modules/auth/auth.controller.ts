...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    // Generate single-use 6-digit OTP valid for 5 minutes only
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    const expiresAt = Date.now() + 5 * 60 * 1000;

    pendingLoginOtps.set(user.email.toLowerCase(), {
      otp,
      userId: user.id,
      userName: user.name,
    ...
    // Dispatch autogenerated login verification OTP email strictly to the user ONLY (valid for 5 mins)
    ...
    return res.status(200).json({
      status: 'otp_required',
      message: 'A 6-digit verification code has been dispatched to your email. Valid for 5 minutes.',
      email: user.email,
      name: user.name
    });
  } catch (err: any) {
