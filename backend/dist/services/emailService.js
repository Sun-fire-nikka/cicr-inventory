...
exports.sendAdminItemCreatedNotification = exports.sendLoginSecurityAlertEmail = exports.sendHardwareRequestStatusEmail = exports.sendAdminHardwareRequestAlert = exports.sendDueReminder = exports.sendReturnReminder = exports.sendUpcomingReminder = exports.sendAdminUserStatusAlert = exports.sendAdminReturnNotification = exports.sendAdminBorrowNotification = exports.sendUserRejectionNotificationEmail = exports.sendUserApprovalSuccessEmail = exports.sendAdminNewUserRegistrationAlert = exports.sendReturnConfirmation = exports.sendLoginOtpEmail = exports.sendOtpEmail = exports.sendBorrowConfirmation = exports.isSmtpConfigured = exports.getSmtpPass = exports.getSmtpUser = exports.DEFAULT_SMTP_PASS = exports.DEFAULT_SMTP_USER = exports.getReplyToAddress = exports.getFromAddress = exports.SUPER_ADMIN_EMAILS = exports.NO_REPLY_HEADER = exports.DEFAULT_SENDER_EMAIL = exports.DEFAULT_TEST_RECIPIENT_EMAIL = exports.SENDER_NAME = void 0;
const dotenv_1 = __importDefault(require("dotenv"));
const nodemailer_1 = __importDefault(require("nodemailer"));
const crypto_1 = __importDefault(require("crypto"));
const emailQueue_1 = require("../config/emailQueue");
dotenv_1.default.config();
// Verified sender + default admin recipients
exports.SENDER_NAME = 'CICR Inventory';
exports.DEFAULT_TEST_RECIPIENT_EMAIL = 'cicrinventory@gmail.com';
exports.DEFAULT_SENDER_EMAIL = 'cicrinventory@gmail.com';
exports.NO_REPLY_HEADER = '"CICR Inventory (No-Reply)" <noreply.cicrinventory@gmail.com>';
...
const getFromAddress = () => {
    ...
    return `"${exports.SENDER_NAME}" <cicrinventory@gmail.com>`;
};
...
const getReplyToAddress = () => {
    ...
    return exports.NO_REPLY_HEADER;
};
exports.getReplyToAddress = getReplyToAddress;
exports.DEFAULT_SMTP_USER = 'cicrinventory@gmail.com';
exports.DEFAULT_SMTP_PASS = 'qbgfgbldvvxxubjx';
const getSmtpUser = () => {
    const envUser = process.env.SMTP_USER;
    if (envUser && !envUser.toLowerCase().includes('kushagra')) {
        return envUser;
    }
    return exports.DEFAULT_SMTP_USER;
};
exports.getSmtpUser = getSmtpUser;
const getSmtpPass = () => {
    const envUser = process.env.SMTP_USER;
    if (envUser && envUser.toLowerCase().includes('kushagra')) {
        return exports.DEFAULT_SMTP_PASS;
    }
    return process.env.SMTP_PASS || exports.DEFAULT_SMTP_PASS;
};
exports.getSmtpPass = getSmtpPass;
const isSmtpConfigured = () => Boolean((0, exports.getSmtpUser)() && (0, exports.getSmtpPass)());
...
const transporter = nodemailer_1.default.createTransport({
    ...
    auth: {
        user: (0, exports.getSmtpUser)(),
        pass: (0, exports.getSmtpPass)(),
    },
});
...
const formatSmtpError = (error) => {
    ...
    if (error?.code !== undefined && error?.code !== null)
        parts.push(`code=${JSON.stringify(error.code)}`);
    if (error?.response)
        parts.push(`response=${JSON.stringify(error.response)}`);
    if (error?.responseCode !== undefined && error?.responseCode !== null)
        parts.push(`responseCode=${JSON.stringify(error.responseCode)}`);
...
const generateMessageId = () => {
    ...
    const idLeft = `${Date.now()}.${crypto_1.default.randomBytes(16).toString('hex')}`;
    return `<${idLeft}@${idRight}>`;
};
// Shared delivery headers for no-reply authentic system appearance
const buildHeaders = (kind, priority = 'normal') => ({
...
const logDelivery = (kind, info) => {
    console.log(`[EMAIL SERVICE] ${kind} accepted by SMTP | messageId=${info?.messageId} | ` +
        `response="${info?.response}" | accepted=${JSON.stringify(info?.accepted || [])} | ` +
        `rejected=${JSON.stringify(info?.rejected || [])}`);
};
const renderCyberEmail = (options) => {
    const badgeColors = {
...
const buildHoldersTable = (holders) => {
    if (!holders.length) {
...
// ──────────────────────────────────────────────────────────────────────────────
const sendBorrowConfirmation = async (recipientEmail, borrowerName, context) => {
    try {
    ...
    `;
        const mailOptions = {
            ...
            replyTo: (0, exports.getReplyToAddress)(),
            to: recipientEmail,
            cc: exports.SUPER_ADMIN_EMAILS.join(', '),
        ...
        if (await (0, emailQueue_1.enqueueEmail)('borrow-confirmation', mailOptions)) {
...
// ──────────────────────────────────────────────────────────────────────────────
const sendOtpEmail = async (adminEmail, adminName, studentName, otp, itemName, durationDays) => {
    try {
        const contentHtml = `
    ...
    `;
        ...
        if (await (0, emailQueue_1.enqueueEmail)('borrow-otp', mailOptions)) {
...
// 3. LOGIN OTP EMAIL
// ──────────────────────────────────────────────────────────────────────────────
const sendLoginOtpEmail = async (recipientEmail, recipientName, otp) => {
    try {
        const contentHtml = `
      <div style="text-align:center;margin:24px 0;">
        <div style="font-family:'SFMono-Regular',Consolas,'Liberation Mono',Menlo,monospace;font-size:28px;font-weight:700;letter-spacing:8px;color:#00f0ff;background:#030712;padding:16px 24px;border:1px solid #1e293b;border-radius:4px;display:inline-block;">
    ...
    `;
        ...
        if (await (0, emailQueue_1.enqueueEmail)('login-otp', mailOptions)) {
            return { success: true, queued: true, otp };
        }
        const info = await transporter.sendMail(mailOptions);
...
// ──────────────────────────────────────────────────────────────────────────────
const sendReturnConfirmation = async (recipientEmail, borrowerName, itemName, returnedAt) => {
    try {
    ...
    `;
        ...
        if (await (0, emailQueue_1.enqueueEmail)('return-confirmation', mailOptions)) {
            return { success: true, queued: true };
        }
        const info = await transporter.sendMail(mailOptions);
...
exports.sendReturnConfirmation = sendReturnConfirmation;
const sendAdminNewUserRegistrationAlert = async (adminEmails, userContext) => {
    try {
        if (!adminEmails || adminEmails.length === 0)
            return { success: false, message: 'No admin recipients provided' };
    ...
    `;
        ...
        if (await (0, emailQueue_1.enqueueEmail)('admin-registration-alert', mailOptions)) {
            return { success: true, queued: true };
        }
        const info = await transporter.sendMail(mailOptions);
...
exports.sendAdminNewUserRegistrationAlert = sendAdminNewUserRegistrationAlert;
// ──────────────────────────────────────────────────────────────────────────────
// 6. USER APPROVAL CONFIRMATION EMAIL
// ──────────────────────────────────────────────────────────────────────────────
const sendUserApprovalSuccessEmail = async (recipientEmail, recipientName) => {
    try {
        const contentHtml = `
      <div style="background:#090c13;border:1px solid #1e293b;border-radius:4px;padding:18px;margin-bottom:18px;">
    ...
    `;
        ...
        if (await (0, emailQueue_1.enqueueEmail)('user-approval-success', mailOptions)) {
            return { success: true, queued: true };
        }
        const info = await transporter.sendMail(mailOptions);
...
exports.sendUserApprovalSuccessEmail = sendUserApprovalSuccessEmail;
// ──────────────────────────────────────────────────────────────────────────────
// 7. USER REJECTION NOTIFICATION EMAIL
// ──────────────────────────────────────────────────────────────────────────────
const sendUserRejectionNotificationEmail = async (recipientEmail, recipientName) => {
    try {
        const contentHtml = `
      <div style="background:#090c13;border:1px solid #1e293b;border-radius:4px;padding:18px;margin-bottom:18px;">
    ...
    `;
        ...
        if (await (0, emailQueue_1.enqueueEmail)('user-rejection', mailOptions)) {
            return { success: true, queued: true };
        }
        const info = await transporter.sendMail(mailOptions);
...
exports.sendUserRejectionNotificationEmail = sendUserRejectionNotificationEmail;
const sendAdminBorrowNotification = async (adminEmails, context) => {
    try {
    ...
    `;
        ...
        if (await (0, emailQueue_1.enqueueEmail)('admin-borrow-alert', mailOptions)) {
            return { success: true, queued: true };
        }
        const info = await transporter.sendMail(mailOptions);
...
exports.sendAdminBorrowNotification = sendAdminBorrowNotification;
const sendAdminReturnNotification = async (adminEmails, context) => {
    try {
        if (!adminEmails || !adminEmails.length)
            return { success: false, message: 'No admin recipients' };
    ...
    `;
        ...
        if (await (0, emailQueue_1.enqueueEmail)('admin-return-alert', mailOptions)) {
            return { success: true, queued: true };
        }
        const info = await transporter.sendMail(mailOptions);
...
exports.sendAdminReturnNotification = sendAdminReturnNotification;
const sendAdminUserStatusAlert = async (adminEmails, userName, userEmail, status, performedBy) => {
    try {
        if (!adminEmails || !adminEmails.length)
            return { success: false, message: 'No admin recipients' };
    ...
    `;
        ...
        if (await (0, emailQueue_1.enqueueEmail)('admin-user-status-log', mailOptions)) {
            return { success: true, queued: true };
        }
        const info = await transporter.sendMail(mailOptions);
...
exports.sendAdminUserStatusAlert = sendAdminUserStatusAlert;
// ──────────────────────────────────────────────────────────────────────────────
// 9. UPCOMING & DUE REMINDERS
// ──────────────────────────────────────────────────────────────────────────────
const sendUpcomingReminder = async (recipientEmail, borrowerName, itemName, dueDate) => {
    try {
    ...
    `;
        ...
        if (await (0, emailQueue_1.enqueueEmail)('upcoming-reminder', mailOptions)) {
            return { success: true, queued: true };
        }
        const info = await transporter.sendMail(mailOptions);
...
exports.sendUpcomingReminder = sendUpcomingReminder;
const sendReturnReminder = async (recipientEmail, borrowerName, itemName, dueDate, daysOverdue) => {
    try {
        const formattedDueDate = formatDueDate(dueDate);
    ...
    `;
        ...
        if (await (0, emailQueue_1.enqueueEmail)('return-reminder', mailOptions)) {
            return { success: true, queued: true };
        }
        const info = await transporter.sendMail(mailOptions);
...
const sendDueReminder = async (recipientEmail, borrowerName, itemName, quantity, dueDate, dueWindowLabel) => {
    try {
        const formattedDueDate = formatDueDate(dueDate);
...
const sendAdminHardwareRequestAlert = async (adminEmails, context) => {
    try {
...
exports.sendAdminHardwareRequestAlert = sendAdminHardwareRequestAlert;
const sendHardwareRequestStatusEmail = async (recipientEmail, borrowerName, itemName, quantity, status, reviewedBy, reason) => {
    try {
        const isApproved = status === 'APPROVED';
        const contentHtml = `
      <div style="background:#090c13;border:1px solid #1e293b;border-radius:4px;padding:18px;margin-bottom:18px;">
...
exports.sendHardwareRequestStatusEmail = sendHardwareRequestStatusEmail;
const sendLoginSecurityAlertEmail = async (context) => {
    try {
    ...
    `;
        const recipients = Array.from(new Set([context.userEmail, ...exports.SUPER_ADMIN_EMAILS]));
        const mailOptions = {
            from: (0, exports.getFromAddress)(),
            replyTo: (0, exports.getReplyToAddress)(),
            to: recipients.join(', '),
        ...
        if (await (0, emailQueue_1.enqueueEmail)('login-security-alert', mailOptions)) {
            return { success: true, queued: true };
        }
        const info = await transporter.sendMail(mailOptions);
...
exports.sendLoginSecurityAlertEmail = sendLoginSecurityAlertEmail;
const sendAdminItemCreatedNotification = async (context) => {
    try {
    ...
    `;
        const adminRecipients = Array.from(new Set([context.createdByAdminEmail, ...exports.SUPER_ADMIN_EMAILS]));
        const mailOptions = {
            from: (0, exports.getFromAddress)(),
            replyTo: (0, exports.getReplyToAddress)(),
            to: context.createdByAdminEmail,
            cc: exports.SUPER_ADMIN_EMAILS.filter(e => e.toLowerCase() !== context.createdByAdminEmail.toLowerCase()).join(', '),
        ...
        if (await (0, emailQueue_1.enqueueEmail)('item-created-telemetry', mailOptions)) {
            return { success: true, queued: true };
        }
        const info = await transporter.sendMail(mailOptions);
...
exports.sendAdminItemCreatedNotification = sendAdminItemCreatedNotification;
